...
object Config {
  private val fileName = System.getProperty("user.home") + File.separator + ".vk-scout"
  private val file = new File(fileName)
  private val RSAKeysFileNamesStr = "RSAKeysFileNames"
  private val EncLoginPasswordStr = "EncLoginPassword"
  private val WebBrowserStr = "WebBrowser"
  private var _rsaKeysFileNames: Option[RSAKeysFileNames] = None
  private var encLoginPasswordInString: Option[EncLoginPasswordInString] = None

  private def rsaKeysFileNames = _rsaKeysFileNames

  private def rsaKeysFileNames_=(optFileNames: Option[RSAKeysFileNames]) {
    _rsaKeysFileNames = optFileNames match {
      case Some(fileNames)
        if new File(fileNames.privateKeyFileName).exists() && new File(fileNames.publicKeyFileName).exists() => optFileNames
      case _ => None
    }
  }
  ...
  private def configFileToString(): String = {
  ...
  def getLoginPassword: Option[LoginPassword] = {
    ...
    saveAndDecodeEncLoginPassword(fieldExtractor[EncLoginPasswordInString](conf, Seq(EncLoginPasswordStr)))
  }

  def saveAndDecodeEncLoginPassword(encLP : Option[EncLoginPasswordInString]) = {
    encLoginPasswordInString = encLP
    decodeEncLoginPassword(encLP)
  }

  def decodeEncLoginPassword(encLP : Option[EncLoginPasswordInString]) = {
    encLoginPasswordInString.map(_.withBytes.decode(rsaTool))
  }

  def save(loginPassword: LoginPassword) {
    try {
      val encLPInStrToSave = decodeEncLoginPassword(encLoginPasswordInString) match {
        case Some(`loginPassword`) =>  encLoginPasswordInString.get
        case _ => loginPassword.encode(rsaTool).withString
      }
      val json = (EncLoginPasswordStr := encLPInStrToSave) ->:
  ...
  private def webBrowser = fieldExtractor[List[String]](configFileToString(), Seq(WebBrowserStr))

  def openInWebBrowser(url: String) {
    webBrowser match {
      case Some(pathToBrowserWithParams) => new ProcessBuilder((pathToBrowserWithParams :+ url).asJava).start()
      case None => Desktop.getDesktop.browse(new URL(url).toURI)
    }
  }
}
